...
procedure: extracted from the actual rename of the reference build
...
- **`scripts/remove-product.mjs`** (if present — post-dates some builds) —
  `const org = "..."` + the hardcoded
...
- **New products** — `pnpm new-product <name>` now stamps correctly (re-proven post-
  ...
  pass `--yes` in non-interactive shells). If your build pre-dates the script, the
  manual inverse is `rm -rf products/<name> && git checkout -- pnpm-lock.yaml
tokens.config.json && pnpm install`.
