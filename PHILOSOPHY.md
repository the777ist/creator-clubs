...
- **Frontend:** Expo SDK 56 (RN 0.85, React 19.2, New Arch mandatory, Hermes v1) + react-native-web, managed + EAS; **NativeWind v4** (v5 is pre-release — do NOT use) on **Tailwind CSS v3** (NOT v4 — v4's CSS-first config belongs to NativeWind v5); Expo Router; **TanStack Query v5 + Zustand v5**; per-platform overrides via `.ios/.android/.web/.native` extensions. **NativeWind-v4 ↔ SDK 56: ✅ VERIFIED WORKING (2026-07-05 Phase 2 run — nativewind 4.2.6 + react-native-css-interop 0.2.6 on expo 56.0.14 / RN 0.85.3 / React 19.2.3, New Arch, Hermes v1; web + android exports both proven). No official pairing exists (last on-record is SDK 54), so on future SDK bumps the safe-harbor fallback remains SDK 54, NOT 55 — fall back only if the interop layer proves unworkable, never preemptively.** **SDK 56 routes global `fetch` through `expo/fetch`** (affects the generated client transport + Sentry network breadcrumbs; escape hatch `EXPO_PUBLIC_USE_RN_FETCH=1`). **EAS Update requires `updates.url` (`https://u.expo.dev/<projectId>`) + a `runtimeVersion` policy in `app.config.ts`** (projectId alone won't deliver OTA)
- **Design system:** **react-native-reusables** (now `founded-labs/react-native-reusables`; run the CLI on the **NativeWind** path, NOT Uniwind/CSS-first) adopted INTO `packages/ui` (shadcn model: components are copied in and OWNED, not a black-box dep); cva variants + `className` escape hatch; **semantic CSS-variable tokens** (`--background`, `--primary`, …) with **light+dark from day 1** (runtime-switchable); per-product branding = each product overrides token VALUES, never component code. `@rn-primitives/*` are now at **1.5.x** (NOT pre-1.0; 1.5.2 as of 2026-07-05) — still pinned exact, but the rationale is **version-coupling to react-native-reusables**, not pre-1.0 instability
...
- **Contracts:** FastAPI OpenAPI → `@hey-api/openapi-ts` (pinned exact, pre-1.0; 0.99.x) + TanStack Query plugin (`@tanstack/react-query`); generated client committed per product. **Do NOT install `@hey-api/client-fetch` as a dependency** — since openapi-ts 0.73 the fetch client is bundled inside `@hey-api/openapi-ts`; `@hey-api/client-fetch` is valid ONLY as a **plugin identifier** in the `plugins` array (on 0.99, both the client and TanStack plugins need `exportFromIndex: true` or the barrel exports neither the shared `client` nor the generated options). Client configured via `setConfig({ baseUrl })` + `interceptors.request.use(...)` — and **`packages/core` never imports a product's generated client by name**: core is shared/unstamped, so the product injects its client into `configureApiClient(client, …)`. FastAPI sets `generate_unique_id_function` (route-name operationIds) so client symbols stay clean
...
│                                  #   + "prettier": "@platform/config/prettier" (ROOT consumes the shared config)
├── eslint.config.mjs              # root re-export of @platform/config/eslint (ESLint 9 resolves from cwd;
│                                  #   lefthook jobs run at repo root — without this every TS/JS commit fails)
├── turbo.json                     # task graph (see below)
├── tsconfig.base.json             # strict, moduleResolution bundler, noEmit
├── .gitignore
├── .prettierignore                # committed generated artifacts (pnpm-lock, openapi.json,
│                                  #   api-client src, supabase .temp/.branches) — generators own their bytes
├── .github/workflows/
...
├── scripts/figma-tokens.mjs       # token source → Style Dictionary v5 → CSS-var theme
│                                  #   files (one-way, committed); reads tokens.config.json
├── tokens.config.json             # TOKEN PIPELINE: fileKey + modes {product → modeId}
├── figma.config.json              # CODE CONNECT CLI config (repo ROOT — a .figma/ subdir is NOT discovered)
├── packages/
│   ├── config/                    # @platform/config: eslint flat config, prettier.json,
│   │   └── ...                    #   tailwind-preset.cjs (design tokens; .cjs — CJS file in an ESM
│   │                              #   package), tsconfig/{base,expo,node}.json
...
**tailwind.config.js:** presets `[require("nativewind/preset"), require("@platform/config/tailwind-preset")]`
(the NativeWind preset is MANDATORY — `withNativeWind` hard-errors without it); content = app
globs + `path.dirname(require.resolve("@platform/ui/package.json")) + "/src/**/*.{ts,tsx}"`
(robust cross-package globs — requires the `"./package.json"` exports passthrough in
`@platform/ui`; `packages/ui` has NO tailwind config for APP consumption — its
workbench-only config serves Storybook's PostCSS pipeline exclusively).
...
react-native-reusables' `@rn-primitives/*` deps exactly (now 1.5.x — pinned for
...
and HERMETIC (inert placeholder env set before importing `main`, which reads Settings at import
time): a clean CI checkout with no `api/.env` must be able to run it.
...
**Typegen:** `@hey-api/openapi-ts` pinned exact (0.99.x; fetch client now bundled in — do
NOT add `@hey-api/client-fetch` as a dep, use it only as a `plugins` identifier, with
`exportFromIndex: true` on the client + TanStack plugin entries) + TanStack Query
plugin (generates `queryOptions`/typed SDK — beats openapi-typescript+openapi-fetch which
needs hand-written glue). Output committed (and prettier-exempt via the root `.prettierignore`
— the generator owns its bytes); CI drift check:
...
| 7 | Generator + stamp `demo` product (brand-asset placeholders + regen script; `pnpm bootstrap`) | `pnpm new-product demo`; both products build via `--affected`; both local stacks run simultaneously via `pnpm bootstrap`; demo carries its own placeholder brand assets; `git grep --untracked -iw template products/demo` empty (`--untracked` — a freshly stamped tree is untracked and plain `git grep` would trivially pass) |
